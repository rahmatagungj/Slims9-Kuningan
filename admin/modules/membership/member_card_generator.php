...
if (isset($_GET['action']) AND $_GET['action'] == 'print') {
	...
	*/
    ...
    $html_str .= '#logo_div{z-index:5;position: absolute;left: 5px;top: 4px;width: 35px;height:35px;}'."\n";
    ...
    foreach ($chunked_card_arrays as $membercard_rows) {
        ...
        foreach ($membercard_rows as $card) {
          ...
          $html_str .= ''.( $sysconf['print']['membercard']['include_id_label']?'':'<!--').'<p class="bio"><label class="bio_label">'.__('Member ID / Tipe').'</label><span>: </span>'.$card['member_id'].' / '.$card['member_type_name'].'</p>'.( $sysconf['print']['membercard']['include_id_label']?'':'-->').'';  
